...
<body>
				...
				<c:choose>
					...
					<c:otherwise>
						<c:forEach var="order" items="${orders}">
							<a href="${pageContext.request.contextPath }/user/sellerInfo?orderId=${order.id}">
    ...
    function deleteOrder() {
        $.getJSON("${pageContext.request.contextPath }/user/sellerOrder/delete/" + ${orderInfo.id} + "&" +${orderInfo.goodId}, function (data) {
            if (data == true){
                alert("交易关闭成功！");
                $(window).attr('location','${pageContext.request.contextPath }/user/userProfile');
            } else {
                alert("有未知原因，交易关闭失败！");
            }
        })
