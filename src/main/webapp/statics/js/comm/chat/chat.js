...
var websocket = null;
var ipAddress = "192.168.3.49";
if ('WebSocket' in window) {
    websocket = new WebSocket("ws://"+ipAddress+":8080/Shop_war/websocket/server");
} else if ('MozWebSocket' in window) {
    websocket = new MozWebSocket("ws://"+ipAddress+":8080/Shop_war/websocket/server");
} else {
    websocket = new SockJS("http://"+ipAddress+":8080/Shop_war/sockjs/server");
}
            ...
            $.post("queryOneUser",
                ...
                function(u){
                    ...
                    $("#recentSession").after("<a class=\"list-group-item session\" onclick=\"startSession("+user.id+")\">\n" +
                        "<img id=\"chatUserIcon"+user.id+"\" src=\""+user.photoUrl+"\" class=\"img-circle sessionIcon\" alt=\"icon\"/>\n" +
                        "<span id=\"unreadNum"+user.id+"\" class=\"badge\" style=\"background-color: #FF0000;\">1</span>\n" +
                        "<b id=\"chatUserName"+user.id+"\">"+user.name+"</b><br>\n" +
